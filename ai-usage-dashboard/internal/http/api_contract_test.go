...
func TestAPIContract_Current(t *testing.T) {
	...
	// Setup test data
	providerID, _ := server.store.CreateProvider("openai", `{}`)
	now := time.Now()
...
func TestAPIContract_Trends(t *testing.T) {
	...
	for _, rangeParam := range validRanges {
		t.Run(rangeParam, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/api/trends?range="+rangeParam, nil)
			w := httptest.NewRecorder()
...
func TestAPIContract_Providers(t *testing.T) {
	...
	// Setup test data
	server.store.CreateProvider("openai", `{"api_key":"***"}`)
	server.store.CreateProvider("anthropic", `{"api_key":"***"}`)
